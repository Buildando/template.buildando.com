...
- `REQ-044`: The platform shall publish a privacy policy describing what the build actually processes, assembled from the configuration surface rather than authored as fixed prose: a section shall appear only when the thing it describes is enabled, per locale where the setting is per locale. When nothing is enabled — no analytics, no ads, no comments embed that can load, no signup form — no policy page, no footer link and no consent banner shall be emitted. Identity values it needs (the contact address) shall come from the configuration surface.
...
Cross-cutting note: a **first automated test batch exists** (`test/`, Vitest, 99
...
- `test/config-integrity.test.ts` — the real configuration surface, checked for
  coherence: every configured locale has a dictionary and the keys the default one
  defines, `ACTIVE_SKIN` names a real skin, nav paths stay locale-agnostic, consent
  stays required while a cookie-setting provider is on. The other unit tests run on
  fixtures so they survive a fork's edits; this one exists to catch edits that do
  not hang together.
- Pure decisions extracted from components so they can be tested at all:
  `facet-routes` (switcher targets for a facet a locale may not have),
  `feed-html` (absolutizing URLs for a feed read off-site), `giscus-theme`,
  `newsletter` (endpoint and provider fields per locale), `privacy` (what a build
  discloses), `search-index` (locale → Pagefind index key), `skins`.
- `test/build.test.ts` — assertions on `dist/` (root redirect, per-locale
  ...
  responsive WebP covers, per-locale markdown hero, the untranslated-post
  fallback page and its suppression once a translation exists, the Pagefind index, the home filter chips and card metadata, the
  ...
  Its subjects are **derived from the build, never named by slug**: the file reads
  the shipped frontmatter and the emitted canonicals to find a post with a cover,
  a draft, a fallback page, a translated pair, and skips the assertion when the
  content has no such case. That is what lets a fork replace the example posts —
  which the install guide tells it to do — without failing the CI that gates its
  own deploy. Verified against a simulated fork carrying a single untranslated,
  cover-less post: 16 passed, 4 skipped, where the slug-bound version failed 9.
...
- `REQ-001`: Done. `src/content/config.ts` (`type: "content"`); a post is `src/content/posts/<slug>/index.md` with colocated images, e.g. `.../o-que-e-o-buildando/`.
...
- `REQ-012`: Done. `BaseHead.astro` + image resolution in `PostLayout.astro`. A post uses its `ogImage`/`cover` when present; otherwise a branded 1200×630 social card is generated at build for `/og/<slug>.png` by `src/pages/og/[...route].ts` (astro-og-canvas, no backend), pulling colors from `BRAND`. The OG route uses `getPublishedPosts`, so drafts get no card. Verified `og:image` is absolute under the domain, the card renders the title + `Site · Category` with correct accented glyphs, and cover-bearing posts still use the cover. The six onboarding posts — three in Portuguese and their English translations — ship designed 1200×630 cover illustrations (dark ground, brand mark, `GUIA`/`GUIDE` eyebrow, title, and a topic glyph: browser, sliders, doc+), a translated pair reusing its counterpart's glyph so the two differ only in their words. `og:image` therefore resolves to the optimized `/_astro/` cover on those. `exemplo-sem-traducao` ships **without** a cover, so the auto-card path has a live example and `test/build.test.ts` asserts it.
...
- `REQ-016`: Done. `src/pages/[lang]/rss.xml.js` emits one feed per locale (`dist/pt/rss.xml`, `dist/en/rss.xml`), announced by an autodiscovery `<link>` in `BaseHead.astro`. Items carry the whole post in `content:encoded`, reusing the `rendered.html` Astro already produced for the page — so markdown, syntax highlighting and image optimization come along and the feed cannot drift from the site. `src/lib/feed-html.ts` absolutizes href/src/poster and every srcset candidate first, since a feed is read off-site where root-relative URLs resolve against the reader.
...
- `REQ-020`: Done. Behind the `src/integrations/Search.astro` port; the Pagefind adapter renders the live-preview UI in the modal, and the `/search` page uses the Pagefind JS API to render results as post cards. The index is **posts-only** — `data-pagefind-body` on the post article (`PostLayout.astro`) excludes category, tag, month, home and about pages. `dist/pagefind/` present; verified in `dist` that posts carry `data-pagefind-body` and other pages do not. Pagefind splits the index by the indexed pages' `<html lang>`, keyed by that tag lowercased (`pt-br`, `en`), so both the modal and the results page pass `src/lib/search-index.ts`'s mapping rather than the locale code — asking for `pt` matched no index and silently answered Portuguese queries from the default one. A build assertion checks each locale asks for an index that was really emitted.
- `REQ-021`: Done. `build` script runs `pagefind --site dist`.
- `REQ-022`: Done (code). Comments now sit behind the `src/integrations/Comments.astro` port (`REQ-043`); the giscus adapter is `src/integrations/comments/giscus/Giscus.astro`, used in `PostLayout.astro`. When `GISCUS.repoId`/`categoryId` are empty the whole section renders nothing (no heading, no script), so an unconfigured template shows no comments UI at all — which is the template's own state. The embed runs live on the `buildando.com` fork, against a dedicated public comments repository. Its theme follows the site's toggle rather than the operating system (`GISCUS.theme = "site"`, mapped in `src/lib/giscus-theme.ts`): the loader resolves both theme names at build time and a `MutationObserver` on `data-theme` pushes `setConfig` to the iframe, since an embed cannot be restyled from outside.
...
- `REQ-044`: Done. `src/pages/[lang]/privacy.astro` assembles the policy from `ANALYTICS`, `INTEGRATIONS`/`GISCUS`, `NEWSLETTER` and `CONSENT`; `src/lib/privacy.ts` decides what applies and whether the page is warranted at all. Comments count only when the embed can really load, so a provider named with empty ids — the template's own state — discloses nothing. Page, footer link and banner follow that single fact, and the page stays out of the sitemap when absent. The contact address lives in `CONSENT.contact`, empty in the template, where the text falls back to the About page. Verified: the template emits no policy and no privacy link; the reference fork emits both, and the English policy said it collected no email until a list existed for that locale.
...
- `REQ-032`: Done. `I18N` in `src/config/site.ts`; Astro `i18n` + root redirect in `astro.config.ts`; string dictionary in `src/i18n/ui.ts` with helpers in `src/i18n/index.ts`; `lang`/`translations` in `src/content/config.ts`; locale-aware routes under `src/pages/[lang]/`; per-locale RSS. The site title and description are localizable too: `SITE.title`/`SITE.description` are the default-locale values, and other locales override them with `site.title`/`site.description` keys in `src/i18n/ui.ts`, consumed via `siteTitle`/`siteDescription` in the home hero, meta description, Open Graph, RSS, and the `WebSite` JSON-LD. Verified in `dist`: root → `/pt/`, per-locale homes/feeds, localized tagline/title/description on the English home, `hreflang` (pt-BR/en/x-default), correct `<html lang>`, language switcher, both locales in the sitemap. Covered by `test/build.test.ts` (root redirect to the configured default, a home and feed per configured locale, sitemap/robots) and `test/i18n.test.ts` (dictionary fallback chain, per-locale title/description overrides, `hreflang` alternates).
- `REQ-033`: Done. `getPostRoutes` in `src/lib/posts.ts` emits each post's canonical route plus a fallback route per untranslated locale, and computes the header switcher targets and hreflang. `src/layouts/PostLayout.astro` splits page locale (`uiLang`, all chrome) from content language (`<article lang>`, tag/category links) and sets the fallback canonical to the source post; `src/components/Header.astro` honors the per-post `langSwitch`. Verified in `dist` with a translated post (switch lands on the translation) and a Portuguese-only post (switch keeps the post, English chrome, `<html lang="en">`, `<article lang="pt-BR">`, canonical → the `/pt/` source). The example content keeps one deliberately untranslated post — `exemplo-sem-traducao` — so the fallback stays exercised end to end even though every other shipped post is now translated; `test/build.test.ts` asserts both it and the fact that a translation suppresses the fallback route. Neither assertion names it: the test locates fallback pages by their canonical pointing out of the locale serving them, so a fork gets the same coverage from its own untranslated posts and this fixture is only what keeps the template's own build covered.
- `REQ-034`: Done. `home` collection in `src/content/config.ts`; per-locale entries `src/content/home/{pt,en}.md`; rendered in the hero by `src/pages/[lang]/index.astro` with a fallback to name + localized description. Covered by `test/build.test.ts`, which reads each locale's `home/<locale>.md`, and asserts inside the rendered `<section class="hero">` both a slug-id heading (proving markdown rendering) and that locale's own prose — scoped to the hero because the same sentence also reaches the meta description, where a hardcoded hero would still satisfy the assertion.
...
- `REQ-039`: Done. `NEWSLETTER` in `src/config/site.ts` (empty by default); `src/components/NewsletterForm.astro` renders a no-JS `<form>` posting to the provider, mounted site-wide in `src/layouts/BaseLayout.astro`, and shown only when configured. `actionUrl` takes one endpoint or one per locale (`src/lib/newsletter.ts`), because the feed is per language and a list fed from it is too — a locale without an endpoint renders no form rather than subscribing a reader to another language's list. `hiddenFields` carries whatever the provider's own form requires (Brevo sends `locale` and an `email_address_check` honeypot); posting without them is accepted and subscribes nobody. With JavaScript the form posts in the background and reports inline, falling back to the native POST otherwise. The reference deployment uses Brevo, one list and one RSS-to-email integration per language. Verified: nothing rendered by default; the form appears, with the provider's fields, exactly where an endpoint is configured.
- `REQ-042`: Done. `CONSENT` in `src/config/site.ts`; `src/integrations/analytics/ConsentBanner.astro` (via the Analytics port in `BaseLayout`) is a provider-agnostic coordinator: cookie-setting adapters register a loader on `window.__consentLoaders`, and it runs the queue only after "Accept" (or immediately when `required: false`), remembering the choice in `localStorage`. Plausible (cookieless) loads directly, ungated. Nothing renders when no gated provider is set. Consent is also withdrawable, which is what makes it consent: a footer control reopens the decision wherever the banner ships, and declining after having accepted expires the analytics cookies across the current URL's ancestor paths and reloads, since a tag already running cannot be unloaded. A CSS fix belongs to the record — `.consent { display: flex }` outranked the browser's `[hidden]` rule, so the banner stayed on screen after a decision until `.consent[hidden]` was added. Verified in a browser: accept loads the tag, the footer control brings the banner back, and declining leaves the page with no tag, no dataLayer and no `_ga` cookies. Also verified: default build has no banner/scripts; with GA+AdSense set no `<script src>` loads before consent; with only Plausible set, it loads with no banner.
- `REQ-038`: Done. `ANALYTICS` in `src/config/site.ts` gained `plausible`, `googleAnalytics`, and `adsense`, all empty by default. The loaders live in the Analytics port's adapters (`src/integrations/analytics/*`), not in `BaseHead.astro`, and register on the consent queue rather than loading directly (`REQ-042`). `BaseHead.astro` emits only AdSense's account-association `<meta>`, which carries no request and no cookie and is therefore ungated — gating it would keep the dashboard from ever verifying the site. `src/components/AdUnit.astro` is an optional manual slot that no page places by default, so an id alone yields automatic formats and nothing else. `ads.txt` belongs to a deployed site, not the template, since it names the publisher. A build assertion checks each provider appears if and only if its id is set.
